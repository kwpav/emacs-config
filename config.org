...
** How it Works
The code here is tangled using =org-babel-tangle=, it creates =init.el= and various "modules" in the =lisp/= directory.
* Startup
...
Although Eglot depends on built-in packages, it needs the newest versions of them. Most of the packages are loaded elsewhere in the config, =eldoc= is all that is left
#+begin_src emacs-lisp :lexical t :tangle lisp/kpav-code.el
  (use-package eldoc
    :straight t)
#+end_src
