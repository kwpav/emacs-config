...
I try to go for an "Emacs first" approach (as opposed to the terminal), so this is meant to be used for Emacs GUI, though it is probably mostly fine in the terminal. I have also only tested this on GNU/Linux and OSX.
** How it Works
...
Display vertico as a separate frame on the bottom, instead of the minibuffer.
#+begin_src emacs-lisp :lexical t :tangle lisp/kpav-narrowing.el
