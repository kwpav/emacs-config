...
** How it Works
As with =basemacs=, this is as a literate config using Org mode, and all configuration is done (mostly) with =use-package=.  All code here is tangled, it creates: =early-init.el=, =init.el=, and various "modules" in the =lisp/= directory.
* Startup
