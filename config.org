...
* About
This is my personal Emacs configuration using [[https://github.com/kwpav/basemacs][basemacs]]  at its core, [[https://github.com/emacs-evil/evil][Evil]] for editing, and [[https://emacs-lsp.github.io/lsp-mode/][LSP Mode]] for coding. It is essentially cobbled together from bits and pieces of other configs I have found online. I have been tweaking it (and will probably continue to tweak it) for several years now. As with =basemacs=, (almost) all configuration is done with =use-package=.  All code here is tangled, it creates: =early-init.el=, =init.el=, and various "modules" in the =lisp/= directory.

I try to go for an "Emacs first" approach (as opposed to the terminal), so this is meant to be used for Emacs GUI, though it is probably mostly fine in the terminal.
* Startup
...
** DONE Fix ~tab~ not expanding headlines in normal mode
...
** DONE Replace this with current config! [2/2]
*** DONE Use it locally
